...
> 3회차는 dev 서버 콜드 컴파일(TTFB 930ms)로 이상값. 1 ~ 2회차 평균은 LCP 약 485ms.

### Step 2 vs Step 3 로컬 비교

| 지표 | Step 2 평균 | Step 3 평균 | 변화 | 비고 |
|------|------------|------------|------|------|
| LCP | 2,378ms | 803ms | **-66.2%** | blur placeholder가 즉시 페인트되어 LCP 후보로 평가됨 |
...
| 로컬 LCP 평균 | 1,743ms | 2,386ms | 2,378ms | 803ms | **-53.9% vs Step 0** |
...
### 프로덕션 검증 (Vercel Preview `/projects/meetup`)

> 측정일: 2026-04-14, 대상 URL: `https://kusitms-g7gfh8pcb-leemanjaes-projects.vercel.app/projects/meetup`

#### 응답 포맷/크기 검증 (샘플 4개)

| 이미지 | 원본 S3 | Vercel 응답 | 절감 | 포맷 변화 |
|--------|--------|-----------|------|----------|
| `on_survey` (로고) | 871,704 B (851KB) PNG | 18,366 B (18KB) | **-97.9%** | PNG → WebP |
| `kucheck` (로고) | 506,446 B (495KB) PNG | 34,358 B (34KB) | **-93.2%** | PNG → WebP |
| `cheese` (로고) | 772,188 B (754KB) PNG | 15,868 B (16KB) | **-97.9%** | PNG → WebP |
| `31th_4_사랑꾼` (포스터) | 273,223 B (267KB) PNG | 16,540 B (16KB) | **-94.0%** | PNG → WebP |
| **평균** | — | — | **~95.7% 절감** | — |

- `Content-Type`: `image/webp` 확인 (Vercel 자동 변환)
- `Cache-Control`: `public, max-age=14400, must-revalidate` (4시간 브라우저 캐시 + Edge 60일)
- `x-vercel-cache: HIT` — 첫 방문자 이후 CDN 캐시 응답 확인

#### Lab 측정 (3회 측정, `/projects/meetup` 프로덕션)

| 측정 | LCP | TTFB | Load Delay | Load Duration | Render Delay | CLS |
|------|-----|------|------------|---------------|--------------|-----|
| 1회차 | n/a | — | — | — | — | 0.00 |
| 2회차 | n/a | — | — | — | — | 0.00 |
| 3회차 | 258ms | 29ms | 27ms | 34ms | 167ms | 0.00 |

> LCP가 보고되지 않은 회차는 리스트 페이지 특성상 viewport 내 큰 LCP 후보 요소가 뚜렷하지 않아 Chrome이 candidate를 확정하지 않은 경우. 3회차 기준 LCP 258ms로 양호.

#### 프로덕션 기준 전체 임팩트 추정

- 메인 페이지 1회 로드 시 S3 이미지 **24개** 요청
- 평균 원본 크기 ~500KB × 24개 = **~12MB** 원본 전송량
- WebP 변환 후 평균 ~20KB × 24개 = **~480KB** 실전송량
- **예상 대역폭 절감: 약 11.5MB / 페이지뷰 (-96%)**

### Step 5 최종 결과

| 항목 | 수치 |
|------|-----|
| 이미지당 평균 바이트 절감 | **-95.7%** (샘플 4개 평균) |
| `/projects/meetup` 페이지뷰당 예상 절감 | **약 11.5MB** (24 이미지 기준) |
| 포맷 전환 | PNG → WebP (자동) |
| CDN 캐시 | Edge 60일, 브라우저 4시간 |
| LCP (프로덕션 3회차) | 258ms |
| CLS | 0.00 |
...
| 2026-04-12 | Step 3 | 핵심 래스터 fill 이미지 3곳에 placeholder="blur" 적용 (로컬 LCP 2,378ms→803ms) |
...
| 2026-04-15 | Step 5 프로덕션 검증 | Vercel Preview에서 샘플 4개 측정: 원본 PNG 267 ~ 851KB → WebP 16 ~ 34KB, 평균 **-95.7% 바이트 절감**, Content-Type `image/webp` 확인 |
