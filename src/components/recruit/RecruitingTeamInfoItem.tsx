  ...
  return (
    ...
    >
      <div className={"flex flex-col gap-y-[40px] items-center justify-center"}>
        <div className={"flex flex-col desktop:flex-row gap-x-[100px] gap-y-[36px] items-center"}>
          {["기획", "디자인", "개발<br/>(프론트엔드,백엔드)"].map((content, idx) => {
            return (
              <div
                key={idx}
                className={`${content == "디자인" ? "desktop:ml-35" : content === "개발<br/>(프론트엔드,백엔드)" ? "desktop:ml-10" : ""} font-extrabold text-[24px] leading-[120%] tracking-[0px] text-center`}
