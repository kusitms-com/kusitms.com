        ...
        <div className="flex flex-col pt-[240px] items-center">
            ...
            >
                ...
                <p className="text-[20px] text-center">
                    KUSITMS은 기획, 개발, 디자인 팀으로 구성되어 있어요 <br />
...
const TeamCard = ({ icon, title, description, reverse, delay }: {
    icon: string;
    title: string;
    description: string;
    reverse?: boolean;
    ...
    return (
        ...
        >
            {!reverse && (
                <Image src={icon} alt={title} width={96} height={96} priority style={{ width: 96, height: 96 }} />
            )}
            <div className={`flex flex-col gap-y-[20px] ${reverse ? "items-end text-end" : ""}`}>
...
const ExecutiveCard = ({ image, title, description, delay, paddingTop }: {
    image: string;
    title: string;
    description: string;
    delay?: number;
    ...
    return (
        ...
        >
            <Image src={image} alt={title} width={347} height={520} priority style={{ width: 347, height: 520 }} />
            <div className={`absolute left-1/2 transform -translate-x-1/2 w-full flex flex-col gap-y-6 items-center justify-center ${paddingTop ? "bottom-[55px]" : "bottom-[185px]"}`}>
                <p className="text-[24px] font-bold">{title}</p>
                <p className="text-center text-[#C5C5D0] whitespace-pre-line">{description}</p>
            </div>
        </motion.div>
    );
};
