...
export const qnaList = [
    ...
    },
    {
        ...
        answer: ["큐시즘은 한 기수동안 재학생, 휴학생, 졸업 유예자처럼 대학생 신분을 유지하는 사람들만 지원이 가능해요."],
    },
    {
        question: "학회 수료시, 수료증을 받을 수 있나요?",
        answer: ["네, 학회의 마지막 커리큘럼에서 서류 기준을 충족한 학회원에게 수료증을 제공하고 있어요."],
    },
    {
    ...
    },
    {
        ...
        answer: [
            ...
            <>
                자세한 이야기는{" "}
                <Link href={"/projects/meetup"} className="text-[#0055FF]">프로젝트 페이지</Link>에서
                확인해보세요.
            </>,],
    },
    {
...
export const recruitItems = [
    ...
    {
        icon: { src: "/recruit/icons/Calendar.svg", alt: "캘린더", width: 44, height: 51 },
        text: (
            <>
                매주 토요일 서울 지역에서 <br />
    ...
    {
        icon: { src: "/recruit/icons/Ranking.svg", alt: "스티커", width: 49, height: 50 },
        text: (
            <>
...
export const recruitInfoList = [
    ...
    },
    {
        ...
        title: "어떤 팀을 모집해요?",
        contents: (
            <div className={"flex flex-col gap-y-[40px] items-center justify-center pt-[72px] pb-[52px]"}>
                <div className={"flex gap-x-[160px]"}>
                    {["기획", "개발", "디자인"].map((content) => {
                        return (
                            <div key={content} className={"font-extrabold text-[24px] leading-[120%] tracking-[0px] text-center"}>{content}</div>
                        );
                    })}
                </div>
                <p className={"text-[#90909A] font-normal text-[16px] leading-[100%] tracking-[-0.5px] text-center"}>2025.01.16~2025.01.25 31기 학회원 모집중이에요 :)</p>
            </div>
        )
    },
    {
        ...
        contents: (
            <div className={"flex flex-col gap-y-[40px] items-center justify-center pt-[72px] pb-[52px]"}>
                <p className={"font-extrabold text-[24px] leading-[120%] tracking-[0px] text-center"}>일시: 매주 토요일 12시
                    ~ 18시 <br/>
                    장소: 서울 지역 내
                </p>
                <p className={"text-[#90909A] font-normal text-[16px] leading-[100%] tracking-[-0.5px] text-center"}>* 활동 장소와 시간은 커리큘럼에 따라 변경될 수 있어요 :)</p>
            </div>
        )
    },
    {
                        ...
                        return (
                            <div key={process.title} className={"relative flex pr-9"}>
                                <div className={"flex flex-col items-center"}>
                                    <div
                                        className={index === recruitmentProcess.length - 1 ? "relative rounded-full" : "relative rounded-full p-5"}>
                                        {index === recruitmentProcess.length - 1 && (<div
                                        ...
                                        <div
                                            ...
                                            <p className={`font-[SUIT] font-extrabold text-[20px] leading-[100%] tracking-[0px] text-center 
                                            ${index % 2 === 0 
                                                ? index === recruitmentProcess.length - 1 
                                                    ? "text-white" 
                                                    : "text-[#0055FF]" 
                                ...
                                <div className={"absolute right-0 top-[25%] flex items-center"}>
                                    {index !== recruitmentProcess.length - 1 && (
                                        <Image className={""} src={"/recruit/icons/BlueArrow.svg"}
                                               alt={"파란색 화살표"} width={32}
                                               height={32}/>
                                    )}
                                </div>
                            </div>
